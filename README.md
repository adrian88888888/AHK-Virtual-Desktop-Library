...
|`FullScreenMode(ExesToIgnore)`|Returns True if you are in full screen mode<br/>Returns False if you are in window mode<br/>ExesToIgnore you can leave it empty, or you can pass an array of winExe´s, it will ignore those, even if they are in full screen it will return false eg. ```ExesToIgnore := ["atom.exe","GitHubDesktop.exe","chrome.exe"]```|
